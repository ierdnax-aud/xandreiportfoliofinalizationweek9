...
Write-Host "Security Protection Test - Nagsisimula..." -ForegroundColor Cyan
Write-Host ("=" * 50)
...
1..120 | ForEach-Object {
    try {
        ...
        $blocked++
        if ($blocked -le 5) { Write-Host "Request $_`: BLOCKED (403)" -ForegroundColor Red }
    }
}
...
Write-Host "  [OK] Rate Limiting: ACTIVE" -ForegroundColor Green
Write-Host "  [OK] Bot Detection: ACTIVE" -ForegroundColor Green
Write-Host "  [OK] SQL Injection Shield: ACTIVE" -ForegroundColor Green
Write-Host "  [OK] XSS Protection: ACTIVE" -ForegroundColor Green
